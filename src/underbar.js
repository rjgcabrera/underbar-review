(function() {
  ...
  _.map = function(collection, iterator) {
    ...
    _.each(collection, function(item, index, arr) {
      results.push(iterator(item, index, arr));
    });
  ...
  _.reduce = function(collection, iterator, accumulator) {
    var result;
    var i;
    var reduceArr = Array.isArray(collection) ? collection : Object.values(collection);

    if (accumulator === undefined) {
      accumulator = reduceArr[0];
      i = 1;
    }
    else {
      i = 0;
    ...
    for (; i < reduceArr.length; i++) {
      result = iterator(result, reduceArr[i]);
    }
  ...
  _.memoize = function(func) {
    ...
    return function(){
      ...
      if (!knownVals.hasOwnProperty(argKey)) {
        knownVals[argKey] = func.apply(this, arguments);
      }
      return knownVals[argKey];
    };
  };
  ...
  _.shuffle = function(array) {
    ...
    while(result.length < array.length) {
      var tmp = array[Math.floor(Math.random() * (array.length))];
      // if (_.indexOf(result, array[tmp]) === -1) {
